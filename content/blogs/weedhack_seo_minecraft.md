...
The fake sites clone the real ones completely: branding, feature lists, FAQs, installation guides, developer credits, and links to the genuine GitHub repos. That last part is a nice touch; linking the real repo makes the clone *more* legitimate-looking than a bare phishing page, and the download button is the only thing that lies. One of the fake sites was built with Lovable, the AI website builder, which tells you exactly how low the production cost has dropped. A convincing multi-page clone with consistent branding is now a prompt away.
...
`cheatlib[.]xyz` is the one that made me laugh: for a tool that doesn't exist at all, inventorying "1.6 million downloads" in the page copy. The lie is load-bearing marketing.

## The distribution math

McAfee's numbers on where the malicious URLs actually lived: 49.6% Discord links, 23.4% MediaFire, 8.2% GitHub. Then the twist, JAR files hosted on Planet Minecart and EndMods, both legitimate Minecraft content sites. The platforms kids already trust and already visit are the CDN. Discord being half the distribution is a generational marker; the support community IS the delivery mechanism, because the links arrive in servers the users already belong to, often shared by accounts that got hacked the same way. It's a worm with a referral program.
